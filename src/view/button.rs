...
mod atomic_button_state {
    ...
    impl AtomicButtonState {
        ...
        pub fn load(&self, order: atomic::Ordering) -> ButtonState {
            // Safety: `AtomicButtonState` is only ever constructed with values casted from valid `ButtonState` values.
...
impl<'cx, UiState> ButtonView<'cx, UiState> {
    pub fn new(ui_context: &UiContext<'cx>, event_router: &EventRouter<'cx, UiState>) -> Self
    where
        UiState: 'cx,
    {
        ...
        let listener_handle = event_router.register_listener(Bounds::default(), dispatch.clone());
        Self {
    ...
    pub fn set_callback(&mut self, callback: ButtonCallback<UiState>)
    where
        UiState: 'cx,
    {
        *self.dispatch.callback.lock().unwrap() = Some(callback);
    }

    pub fn with_callback(mut self, callback: ButtonCallback<UiState>) -> Self
    where
        UiState: 'cx,
    {
    ...
    pub fn set_title(&mut self, title: impl Into<Cow<'cx, str>>) {
        self.text_view.set_text(title);
        self.relayout_text();
    }

    pub fn with_title(mut self, title: impl Into<Cow<'cx, str>>) -> Self {
...
struct ButtonDispatch<UiState> {
    ...
    callback: Mutex<Option<ButtonCallback<UiState>>>,
}
